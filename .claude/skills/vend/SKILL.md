...
当用户输入 `/vend` 或直接说出以下意图时触发本 SKILL：
- "/vend"
- "帮我搞钱"
- "帮我挣钱"
- "帮我赚钱"
...
> **运行时根目录 = `docs/`**：所有数据（config、产品、日志、历史）均位于仓库根的 `docs/` 目录下。历史文档中出现的 `vend/`、`make-money-skill/` 均为旧路径，一律视为 `docs/`。

读取配置文件：`docs/config.json`

> **文件内容优先级**：`config.json` 和 `SKILL.md` 的内容优先级高于 `docs/` 目录下其它任何文件。如果其它文件内容与 `config.json` 或 `SKILL.md` 存在矛盾，一律以 `config.json` 和 `SKILL.md` 为准。

配置项示例（仅展示结构，金额为占位，**以 `docs/config.json` 实际值为准**）：
...
- `assets`: 本地资产目录
  ...
  - `log_dir`: 搞钱日志目录
- `accounts`: 各平台账号凭证
  - **默认渠道（用户环境已验证可用，详见 memory `available-channels`）**：
    - 上架收款（海外/全球）：`payloadz`（`email`, `password`）→ 提现到 `PayPal`（`email`, `password`，主收款身份）
    - 上架收款（国内）：`xiaohongshu`（小红书店铺 / 个人售卖，支付宝/微信收款，适合国内用户，无需跳转外部站点）
    - 引流：`twitter`（X）、`instagram`、`youtube`、`xiaohongshu`（小红书），各含 `email`/`username`/`password`
  - **不可用（不要再依赖）**：`gumroad`（上架）、TikTok（引流）—— 用户环境不支持，已二次确认
  - `twitter` 额外含 API 字段：`api_key`, `api_secret`, `access_token`, `access_token_secret`
  - 其余平台（Sellfy、ko-fi.com、itch.io、payhip、任推帮、terabox、Google、hashnode 等）**未验证**，仅在用户确认可用后按各平台所需字段存放
  - **注意**：账号密码等敏感信息只保存在本地 `config.json` 中，不要写回 `SKILL.md` 或日志
- `llm_api_keys`: 多个大模型 API Key / 服务凭据
  - 当当前模型能力不足或需要调用外部模型服务时，可使用这些 Key
  - 标准大模型服务：`openai`、`anthropic`、`gemini`、`openrouter`
  - `Agnes`：第三方 LLM 中转/代理服务 Key
  - `devto_api_key`：Dev.to 发布文章用的 API Key
  - 键名为服务标识，值为对应的 API Key
  - **可选**：如需 NVIDIA integrate API（多模态、文本/图片/视频），可在 config 中自行新增 `nvidia` 段（含 `api_key`(`nvapi-` 开头)、`invoke_url`、`model`、`max_tokens`/`temperature`/`top_p`、`supports_stream`/`supports_multimodal`）；现行 `docs/config.json` 未配置此项
...
2. **需要用户授予或输入密码**
   ...
   ### 手机验证（Phone Verification）处理规则
   当通过浏览器自动化操作平台（如 Twitter/X、Reddit、Quora 等）时，可能触发手机验证。处理流程：
   1. **检测到手机验证页面**后，立即**停止自动化操作**，不要尝试跳过或绕过
   2. **保持浏览器窗口打开**，截图记录当前状态
   3. **通知用户**：告知哪个平台触发了手机验证，请求用户在浏览器窗口中手动完成验证
   4. **等待用户完成**：设置 60-120 秒等待时间，让用户在浏览器中输入手机验证码
   5. **验证完成后继续**：检测到验证通过后，继续执行自动化操作
   6. **不重试验证**：如果验证失败，不要反复触发，改为询问用户是否换其他方式
...
- **速度优先**：在合法合规前提下，优先选择能最快产生收入的路径。注意：此处的「速度」指自有项目的内部效率 KPI（即 `speed` 配置的 200 HKD/小时），**不等于**去接时薪/日薪外包——后者仍属「不要按时间计费」要拒绝的范畴
...
## NVIDIA integrate API 调用规范（可选）

> **现状提示**：现行 `docs/config.json` **未配置** `nvidia`。以下内容仅当你在 config 中新增 `llm_api_keys.nvidia` 段后才适用，作为调用参考保留。

`llm_api_keys.nvidia` 用于调用 NVIDIA integrate 提供的 OpenAI 兼容 Chat Completions 接口（例如 `minimaxai/minimax-m3` 等多模态模型）。调用时从 `docs/config.json` 读取真实凭据，不要在代码或日志里硬编码 API Key。
...
config = json.loads(Path("docs/config.json").read_text())
